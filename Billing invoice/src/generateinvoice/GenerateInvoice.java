...
public class GenerateInvoice{
	 ...
	 public GenerateInvoice(String filename,String[] client,String[] invoice,String projectname,Object[] pair){
		 String pdfFilename = "";
		 pdfFilename = filename+".pdf".trim();
		 createPDF(pdfFilename,client,invoice,projectname,pair);
	 }

	 private void createPDF (String pdfFilename,String[] client,String[] invoice,String projectname,Object[] pair){
		 Document doc = new Document();
		 PdfWriter docWriter = null;
		 initializeFonts();
		 try {
			 String path = "C:/Users/sowmya/Desktop/" + pdfFilename;
			 ...
			 int y = 0;
			 ArrayList developer=(ArrayList)pair[1];
			 for(int i=0; i < developer.size(); i++ ){
				 ...
				 generateDetail(doc, cb, i, y, tokens,(String)pair[0]);
				 y = y - 15;
				 if(y < 50){
					 printPageNumber(cb);
					 doc.newPage();
	 ...
	 private void generateDetail(Document doc, PdfContentByte cb, int index, int y, String[] tokens,String date)  {
		 DecimalFormat df = new DecimalFormat("0.00");
		 try{
			 createContent(cb,105,y,date,PdfContentByte.ALIGN_RIGHT);
			 createContent(cb,150,y, tokens[1],PdfContentByte.ALIGN_LEFT);
			 createContent(cb,400,y, tokens[2],PdfContentByte.ALIGN_LEFT);
			 createContent(cb,470,y, tokens[3],PdfContentByte.ALIGN_RIGHT);
			 createContent(cb,525,y, ""+(Integer.parseInt(tokens[3])*Integer.parseInt(tokens[2])),PdfContentByte.ALIGN_RIGHT);		 
