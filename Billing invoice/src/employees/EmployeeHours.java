...
public class EmployeeHours implements ActionListener{
	...
	public void addEmployeeHours(JPanel contentPane,User user){
		...
		tempUser=user;
		JLabel headingLabel = new JLabel("ADD EMPLOYEE WORK HOURS");
		headingLabel.setForeground(Color.BLACK);
		headingLabel.setFont(new Font("Times New Roman", Font.BOLD, 18));
		headingLabel.setBounds(200, 150, 350, 40);
		contentPane.add(headingLabel);
		System.out.println(user.getUsername()+"  User Information   "+user.getUserrole());
		ArrayList projectList=null;
		if(user.getUserrole().equalsIgnoreCase("project manager")){
			projectList=service.getAllProjectManagerProjects(tempUser.getUsername());
		}else{
			projectList=service.getAllDeveloperProjects(tempUser.getUsername());
		}
		if(projectList!=null && projectList.size()!=0){
			...
			selectdateButton.addActionListener(new ActionListener(){	
				public void actionPerformed(ActionEvent arg0){
					...
					day.setText(new SelectDatePicker(f).setSelectedDate());
				}
			});
			selectdateButton.setBounds(505, 230, 150, 25);
			contentPane.add(selectdateButton);
			
//			headingLabel = new JLabel("Ex : DD-MM-YYYY");
//			headingLabel.setFont(new Font("Times New Roman", Font.BOLD, 14));
		...
		if(e.getActionCommand().equalsIgnoreCase("Save")){
			if(day!=null && hours!=null){
				if(day.getText().trim().length()!=0 && hours.getText().trim().length()!=0){
					ArrayList projectList=null;
					if(tempUser.getUserrole().equalsIgnoreCase("project manager")){
						projectList=service.getAllProjectManagerProjects(tempUser.getUsername());
					}else{
						projectList=service.getAllDeveloperProjects(tempUser.getUsername());
					}
