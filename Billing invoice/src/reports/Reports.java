...
public class Reports implements ActionListener{
	...
	public void budgetReport(JPanel contentPane,User user){
		...
		contentPane.add(headingLabel);	
		ArrayList projectList=service.invoicesReport();
		String clientColumnNames[] = {"Client Name", "Project Number", "Frequnecy", "Billing Terms", "Invoice Date", "Initial Budget","Paid Budget", "Balance Budget"};
		int columnsCount=8;			
		String projectRowColumnDataValues[][]=new String[0][columnsCount];		
		if(projectList!=null && projectList.size()!=0){
			projectRowColumnDataValues=new String[projectList.size()][columnsCount];		
			...
			while(iterator.hasNext()){
				...
				projectRowColumnDataValues[poistion][2]=tokens[3];
				projectRowColumnDataValues[poistion][3]=tokens[4];
				projectRowColumnDataValues[poistion][4]=tokens[5];
				String[] project=service.getProjectData(tokens[7], tokens[2]);
				projectRowColumnDataValues[poistion][5]=project[8];
				projectRowColumnDataValues[poistion][6]=tokens[6];
				String spentmoney=service.getProjectPaidAmount(tokens[2], tokens[5]);
				projectRowColumnDataValues[poistion][7]=""+(Double.parseDouble(project[8])-Double.parseDouble(spentmoney));
				poistion=poistion+1;
			}			
		}
		final JTable clientTable = new JTable(projectRowColumnDataValues, clientColumnNames);	        
	...
	public void payrollReport(JPanel contentPane,User user){
		...
		headingLabel.setBounds(250, 150, 350, 40);
		contentPane.add(headingLabel);		
		String clientColumnNames[] = {"Project Number", "Developer Name", "Bill Rate", "Worked Date", "Worked Hours", "Approve Status"};
		...
		if(projectList!=null && projectList.size()!=0){
			...
			while(iterator.hasNext()){
				...
				projectRowColumnDataValues[poistion][3]=tokens[3];
				projectRowColumnDataValues[poistion][4]=tokens[4];
				if(tokens[7].equalsIgnoreCase("Y")){
					projectRowColumnDataValues[poistion][5]="Approved";
				}else{
					projectRowColumnDataValues[poistion][5]="Not Approved";
				}
				poistion=poistion+1;
			}			
		}
